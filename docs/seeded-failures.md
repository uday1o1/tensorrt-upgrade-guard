...
The candidate-aware reducer keeps the G2 output, narrows the failing shape to one row at hidden size 259, simplifies finite inputs and builder options, and retains the first passing-to-failing locked environment boundary.
Every accepted stage re-executes the same typed three-way numerical predicate through the locked worker boundary.

The exported source-bearing bundle retains the exact reduced model, profile, inputs, numerical policy, environments, predicate identity, source build recipe, and original failure evidence.
Its clean replay rebuilds the worker and engine from a new empty directory and must observe the original failure code and signature.
...
The runner gathers 24 hardware-valid adjacent observations with a locked balanced schedule containing 12 observations in each execution order.

The performance reducer cannot use a single slow observation.
It searches for a smaller valid subset, retains at least 20 accepted pairs in a hash-addressed artifact, and requires the one-sided lower bootstrap bound to exceed the locked 3 percent practical allowance.
The final subset is checked for the locked confirmation count from a new empty directory before publication.
